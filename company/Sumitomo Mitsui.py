...
def get_jobs():
    ...
    bs_text = BeautifulSoup(response.content, 'html.parser').text
    pagesize = int(re.compile(r'Results \d+ to (\d+)').findall(bs_text)[0])
    pages = int(re.compile(r'Page \d+ of (\d+)').findall(bs_text)[0])
    pages = [f"{meta['urls']['page']}{i*pagesize}" for i in range(pages)]
